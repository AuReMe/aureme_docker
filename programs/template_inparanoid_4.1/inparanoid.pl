...
$blastall = "/programs/blast-2.2.17/bin/blastall";  #Add -aN to use N processors
$formatdb = "/programs/blast-2.2.17/bin/formatdb";
$seqstat = "seqstat.jar";
...
if ($output){
    open OUTPUT, ">$outputfile" or warn "Could not write to OUTPUT file $outputfile\n";
}
...
sub bootstrap{
    ...
    # print "FORMATDB: $formatdb -i $db_file\n";
    system "$formatdb -i $db_file";
    # Use soft masking in 1-pass mode for simplicity.
    system "$blastall -F\"m S\" -i $query_file -z 5000000 -d $db_file -p blastp -M $matrix -m7 | ./unquote.sh | ./$blastParser 0 -a > $seq_id2.faa";
...
sub do_blast_1pass {
  ...
  system ("$blastall -F\"m S\" -i $Fld[0] -d $Fld[1] -p blastp -v $Fld[3] -b $Fld[3] -M $matrix -z 5000000 -m7 | ./unquote.sh | ./$blastParser $score_cutoff > $Fld[4]");
}
...
sub do_blast_2pass {
	...
	system("date");
	open FHR, "$blastall -C3 -F\"m S\" -i $Fld[0] -d $Fld[1] -p blastp -v $Fld[3] -b $Fld[3] -M $matrix -z 5000000 -m7 | ./unquote.sh | ./$blastParser $score_cutoff|";
	print "[[[]]] $blastall -C3 -F\"m S\" -i $Fld[0] -d $Fld[1] -p blastp -v $Fld[3] -b $Fld[3] -M $matrix -z 5000000 -m7 | ./unquote.sh | ./$blastParser $score_cutoff|";
	...
	foreach $aQuery (keys % theHits) {
		...
		# print "FORMATDB2: $formatdb -i $tmpd\n";
		system ("$formatdb -i $tmpd");
		# die "FORMATDB for tmpd ready. Check!\n";
		system ("$blastall -C0 -FF -i $tmpi -d $tmpd -p blastp -v $Fld[3] -b $Fld[3] -M $matrix -z 5000000 -m7 | ./unquote.sh | ./$blastParser $score_cutoff >> $Fld[4]");
	}
